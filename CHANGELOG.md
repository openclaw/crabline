...
- Revalidate recorder hardlink identities, persist full path durability on every append, recover recycled recorder-owner PIDs, departed runtime namespaces, and terminated worker contexts without stealing live or legacy locks, preserve shared Windows coordination behind private local locks, fence partial recovery-chain cleanup, and create or migrate owner-only Windows lock roots.
