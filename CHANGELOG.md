...
- Revalidate recorder hardlink identities, persist full path durability on every append, recover recycled recorder-owner PIDs and departed runtime namespaces without stealing live or legacy locks, preserve shared Windows coordination behind private local locks, and create or migrate owner-only Windows lock roots.
