...
- Revalidate recorder hardlink identities, persist full path durability on every append, recover recycled recorder-owner PIDs, departed runtime namespaces, terminated worker contexts, interrupted owner publication, and stale pre-upgrade locks behind durable recovery claims, preserve machine fencing and shared Windows coordination behind private local locks, retry transient release coordination, fall back to non-attach Darwin process identities, use identity-verified Windows tail repair, fence partial recovery-chain cleanup, and create or migrate owner-only Windows lock roots.
